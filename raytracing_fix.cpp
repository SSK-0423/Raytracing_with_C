...
int main()
{
    ...
    // 球
    Shape *geometry = new Sphere(Vector3(0, 0, 5), 1.f);
    ...
    for (int y = 0; y < bitmap.height; y++)
    {
        for (int x = 0; x < bitmap.width; x++)
        {
            ...
            // レイと球の交点
            IntersectionPoint *intersectionPoint = geometry->isIntersectionRay(&ray);
            ...
            if (intersectionPoint != nullptr)
            {
                ...
                recordLine("交点の位置ベクトル (%4.2f,%4.2f,%4.2f)\n",
                           intersectionPoint->position.x,
                           intersectionPoint->position.y,
                           intersectionPoint->position.z);
    ...
    // PNGに変換してファイル保存
    if (pngFileEncodeWrite(&bitmap, "raytracing_fix.png") == -1)
    {
